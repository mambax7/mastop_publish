...
<div class="panel_wrapper">
    ...
    ';
            foreach (array_keys($images) as $i) {
                ...
                echo '<img src="' . $imagem_url . '" alt="" width="50" onmouseover="this.style.border=\'2px solid black\'"  onmouseout="this.style.border=\'2px solid white\'" style="border:2px solid white" onclick="addItem(\'' . $imagem_url
                     . '\', \'' . $images[$i]->getVar('image_nicename') . '\', \'' . $images[$i]->getVar('image_nicename') . '\')"/>';
                ...
                echo '<td style="border: 2px double #F0F0EE; text-align: center"><a href="javascript:void(0)" onclick="addItem(\'' . $imagem_url . '\', \'' . $images[$i]->getVar('image_nicename') . '\', \'' . $images[$i]->getVar('image_nicename')
                     . '\')">' . _SELECT . '</a></td></tr>';
            }
            echo '</tbody></table>';
            if ($imgcount > 0) {
    ...
    <?php if ($admin) {
        ?>
        <div id="nova_cat_panel" class="panel <?php echo ($op === 'addcat' || $readCount <= 0) ? 'current' : '';
        ?>" style="overflow: auto;">
    ...
    <?php if ($writeCount > 0) {
        ?>
        <div id="nova_imagem_panel" class="panel <?php echo ($op === 'addfile') ? 'current' : '';
        ?>" style="overflow: visible;">
            ...
            if ($op === 'addfile') {
                ...
                $imagecategory =& $imgcat_handler->get((int)$_POST['imgcat_id']);
                if (!is_object($imagecategory)) {
                    redirect_header($_SERVER['PHP_SELF'], 1);
                }
    ...
    ';
                    ...
                    echo '<img src="' . $imagem_url . '" alt="" width="50" onmouseover="this.style.border=\'2px solid black\'"  onmouseout="this.style.border=\'2px solid white\'" style="border:2px solid white" onclick="addItem(\'' . $imagem_url
                         . '\', \'' . $image->getVar('image_nicename') . '\', \'' . $image->getVar('image_nicename') . '\')"/>';
                    ...
                    echo '<td style="border: 2px double #F0F0EE; text-align: center"><a href="javascript:void(0)" onclick="addItem(\'' . $imagem_url . '\', \'' . $image->getVar('image_nicename') . '\', \'' . $image->getVar('image_nicename') . '\')">'
                         . _SELECT . '</a></td></tr>';
                }
