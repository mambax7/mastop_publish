...
if (!class_exists('Mastop')) {
    class Mastop extends \XoopsObject
    {
        ...
        public function administracao($url, $campos)
        {
            ...
            $busca_url = '';
           if (\Xmf\Request::hasVar('busca', 'GET')) {
            ...
            $order = \Xmf\Request::getString('order', 'DESC', 'GET');
            $criterio->setOrder($order);            
            $sort = (\Xmf\Request::hasVar('sort', 'GET') && in_array($_GET['sort'], $campos['nome'])) ? $_GET['sort'] : (empty($campos['sort']) ? $campos['nome'][1] : $campos['sort']);
